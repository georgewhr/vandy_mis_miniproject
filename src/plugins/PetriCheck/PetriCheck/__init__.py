...
The PetriCheck-class is imported from both run_plugin.py and run_debug.py
"""
import sys
import logging
...
# Setup a logger
logger = logging.getLogger('PetriCheck')
...
class PetriCheck(PluginBase):
    def main(self):
        ...
        states = set()
        graph = {}
        self.send_notification('hello george')
        logger.info("georgewhr in PetriCheck")
        ...
        if len(states.difference(visited)) == 0:
            # everything is fine
            self.send_notification('It is a StateMachine')
        else:
